...
async function run() {
  try {
    ...
    app.post("/book-marks", verifyJWT, async (req, res) => {
      ...
      const isAvailable = await bookMarksCollection.findOne({
        postId: id,
        email: bookMarks.email,
      });
      if (!isAvailable) {
    ...
    app.post("/conversations", async (req, res) => {
      ...
      try {
        ...
        res
          .status(200)
          .send({ message: "Conversation saved successfully", result });
      } catch (error) {
        console.error("Error saving conversation:", error);
        res
          .status(500)
          .send({ error: "An error occurred while saving the conversation" });
      }
    });
    ...
    app.patch(
      ...
      async (req, res) => {
        ...
        const senderCollections = await connectionsCollection.findOne({
          email: data.sender,
        });
        const receiverCollections = await connectionsCollection.findOne({
          email: data.receiver,
        });
        ...
        const result1 = await connectionsCollection.updateOne(
          { email: data.sender },
          updateDoc
        );
        const result2 = await connectionsCollection.updateOne(
          { email: data.receiver },
          updateDoc1
        );
    ...
    app.post("/payment/success/:transaction_Id", async (req, res) => {
      ...
      const result = await sslPaymentsCollection.updateOne(
      ...
      if (result.modifiedCount > 0) {
        res.redirect(
          `https://insight-space-f2643.web.app/payment/success/${req.params.transaction_Id}`
        );
      }
    });

    app.post("/payment/fail/:transaction_Id", async (req, res) => {
      const result = await sslPaymentsCollection.deleteOne({
        transaction_Id: req.params.transaction_Id,
      });

      if (result.deletedCount) {
        res.redirect(
          `https://insight-space-f2643.web.app/payment/fail/${req.params.transaction_Id}`
        );
      }
    });
    ...
    app.post("/payments", verifyJWT, async (req, res) => {
      ...
      if (instructorRequest) {
        const availablePayment = await paymentCollection.findOne({
          email: payment.email,
        });
        if (!availablePayment) {
    ...
    app.get("/exam-test", async (req, res) => {
      const email = req.query.email;
      const query = { email: email };
      const result = await quizExamCollection
        .find(query)
        .sort({ date: -1 })
        .toArray();
      res.send(result);
    ...
    socketIO.on("connection", (socket) => {
      ...
      socket.on("getConversationData", async (userId) => {
        ...
        const conversationUserData = Promise.all(
          conversations.map(async (conversation) => {
            ...
            const conversationUserId = conversation.members.find(
              (m) => m !== userId
            );
      ...
      socket.on("chatMessage", async (messageData) => {
        ...
        const messages = await messageCollection
          .find({ conversationId: messageData.conversationId })
          .toArray();
